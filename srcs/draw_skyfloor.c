...
/*   Updated: 2020/01/06 17:00:08 by rofernan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_tex_coor(t_cub3d *cub)
{
	...
	if (cub->tex[4].texture == 1)
	{
		...
		while (y < cub->res_y)
		{
			...
			ft_memmove(&cub->img_ptr[(cub->res_x * y + x) * \
						cub->tex[4].bit_pix / 8], \
						&cub->tex[4].img_ptr[cub->flr.fl_tex], sizeof(int));
			y++;
		}
	}
	else
...
void		draw_ceiling(t_cub3d *cub, int x)
{
	...
	if (cub->tex[5].texture == 1)
	{
		while (y < cub->draw_start)
		{
			...
			ft_memmove(&cub->img_ptr[(cub->res_x * y + x) * \
						cub->tex[5].bit_pix / 8], \
						&cub->tex[5].img_ptr[cub->flr.fl_tex], sizeof(int));
			y++;
		}
	}
	else
